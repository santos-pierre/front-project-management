...
const ProjectItem = ({ project, onClick }: ProjectItemProps) => {
    return (
        <Link
            to={getRoute('projects-show', { slug: project.slug }).path}
            key={project.slug}
            ...
            <li className="relative z-auto py-5 pl-4 pr-6 hover:bg-gray-50 sm:py-6 sm:pl-6 lg:pl-8 xl:pl-6">
                <div className="flex items-center justify-between space-x-4">
                    <div className="min-w-0 space-y-3">
                        ...
                        {project.repository_url && (
                            <span className="relative group flex items-center space-x-2.5">
                                ...
                                <div className="text-sm font-medium leading-5 text-gray-500 truncate group-hover:text-gray-900">
                                    {project.repository_url}
                                </div>
                            </span>
                        )}
                    ...
                    <div className="flex-col items-end flex-shrink-0 hidden space-y-3 sm:flex">
                        <p className="flex flex-col items-center space-x-2 space-y-2 text-sm leading-5 text-gray-500">
                            <svg
                                className="w-5 h-5"
                                fill="currentColor"
                                viewBox="0 0 20 20"
