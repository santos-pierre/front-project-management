...
const InputForm = React.forwardRef<Ref, InputFormProps>(({ name, label, type, id, error, placeholder, children }, ref) => {
    const inputStyles = {
    ...
    return (
        <div className={name}>
            ...
            {error && <p className="mt-2 ml-2 text-sm text-red-600">{error}</p>}
        </div>
    )
});
